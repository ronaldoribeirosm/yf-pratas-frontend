...
export function CartProvider({ children }) {
  ...
  const fetchCartFromAPI = async (token) => {
    try {
      const response = await fetch('https://yf-pratas-backend.onrender.com/carrinho', {
  ...
  const addToCart = async (product) => {
    ...
    try {
      const res = await fetch('https://yf-pratas-backend.onrender.com/carrinho', {
        method: 'POST',
        headers: {
  ...
  const removeFromCart = async (productId) => {
    ...
    if (token) {
      try {
        const res = await fetch(`https://yf-pratas-backend.onrender.com/carrinho/${productId}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${token}` }
        });
